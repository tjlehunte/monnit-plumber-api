...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let activeChartType = "monnit";
  let customLegendCollapsed = true;
  ...
  function showMonnitChart() {
    const monnitArea = document.getElementById("monnitChartArea");
    const givenergyArea = document.getElementById("givenergyChartArea");
    const legendControls = document.getElementById("legendControls");
    monnitArea.style.display = "block";
    givenergyArea.style.display = "none";
  ...
  function showGivenergyChartCanvas() {
    ...
    const givenergyArea = document.getElementById("givenergyChartArea");
    const legendControls = document.getElementById("legendControls");
    monnitArea.style.display = "none";
    givenergyArea.style.display = "block";
  ...
  function populateCustomLegend(chart) {
    ...
    const visibleDatasets = customLegendCollapsed ? datasets.slice(0, 6) : datasets;

    visibleDatasets.forEach((dataset, index) => {
      const item = document.createElement("button");
      const color = Array.isArray(dataset.backgroundColor) ? dataset.backgroundColor[0] : dataset.backgroundColor;
      item.type = "button";
      item.className = "legend-item";
      if (!chart.isDatasetVisible(index)) item.classList.add("is-hidden");
      item.innerHTML = `
    ...
    if (datasets.length > 6) {
      ...
      toggle.textContent = customLegendCollapsed ? `Show ${datasets.length - 6} more` : "Show fewer";
      toggle.addEventListener("click", () => {
        customLegendCollapsed = !customLegendCollapsed;
        populateCustomLegend(chart);
      });
  ...
  function updateFooter() {
    ...
    if (!lastUpdated || !allData.length) return;
    const lastItem = allData[allData.length - 1];
    lastUpdated.textContent = `Last updated: ${lastItem.MessageDate}`;
  }
  ...
  function drawChart(data, cols, title, unit, isCurrentChart = false) {
    ...
    mainChart.options.plugins.legend.display = !isMobile();
  ...
  function initTabs() {
    ...
    document.querySelectorAll("#masterTabs .tab").forEach(tab => {
      tab.addEventListener("click", () => {
        ...
        // If "givenergy" tab clicked, show GiveEnergy sub-tabs
        if (master === "givenergy") {
          document.getElementById("envTabs").style.display = "none";
          document.getElementById("currentTabs").style.display = "none";
          document.getElementById("givenergyTabs").style.display = "flex";
          showGivenergyChartCanvas();
          // Trigger the currently active GiveEnergy sub-tab to draw its chart
          const activeGivenergy = document.querySelector("#givenergyTabs .tab.active");
          if (activeGivenergy) activeGivenergy.click();
        }
      });
    });
  ...
  document.getElementById("hideAllBtn").onclick = () => {
    ...
    activeChart.data.datasets.forEach((_, i) => activeChart.setDatasetVisibility(i, false));
    activeChart.update();
    populateCustomLegend(activeChart);
  };
  ...
  document.getElementById("showAllBtn").onclick = () => {
    ...
    activeChart.data.datasets.forEach((_, i) => activeChart.setDatasetVisibility(i, true));
    activeChart.update();
    populateCustomLegend(activeChart);
  };
  ...
  document.getElementById("invertBtn").onclick = () => {
    ...
    activeChart.data.datasets.forEach((_, i) => activeChart.setDatasetVisibility(i, !activeChart.isDatasetVisible(i)));
    activeChart.update();
    populateCustomLegend(activeChart);
  };
  ...
  async function loadData() {
    ...
    if (!givenergyChart) {
      ...
      givenergyChart = new Chart(ctx3, {
        ...
        options: {
          ...
          plugins: {
            legend: { display: false, labels: { color: textColor } },  // No legend for single dataset
            tooltip: { enabled: true },                 // Show data on hover
    ...
    if (activeMaster && activeMaster.dataset.master === "givenergy") {
      showGivenergyChartCanvas();
      const activeSub = document.querySelector("#givenergyTabs .tab.active");
      if (activeSub) activeSub.click();
    } else if (activeMaster && activeMaster.dataset.master === "current") {
      showMonnitChart();
